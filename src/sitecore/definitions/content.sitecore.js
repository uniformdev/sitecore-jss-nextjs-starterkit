...
export default function addContentToManifest(manifest) {
  const rootItemName = 'global';
  const rootItemDisplayName = 'Global Content';
 ...
 */
function convertToItems(data, basePath, rootItemName, rootItemDisplayName, language) {
  ...
  if (contentFileData && contentFileData.contents) {
    ...
    result = {
      ...
      displayName: name || rootItemDisplayName || rootItemName,
      template: 'Folder',
      children: [],
    };
  }
  ...
  if (data.folders.length > 0) {
    result.children = data.folders
      .map((folder) =>
        convertToItems(folder, basePath, rootItemName, rootItemDisplayName, language)
      )
